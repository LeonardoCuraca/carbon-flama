...
export default function Sidebar() {
  ...
  return (
    <div className={`bg-[#141414] border-r border-white/5 flex flex-col h-screen sticky top-0 transition-all duration-300 ease-in-out relative ${isCollapsed ? "w-20" : "w-64"
      }`}>
      {/* Header del Sidebar */}
      <div className="p-6 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <button
            ...
            className={`w-8 h-8 bg-orange-600 rounded-lg flex items-center justify-center flex-shrink-0 ${isCollapsed ? "cursor-pointer hover:bg-orange-500 transition-colors" : "cursor-default"
              }`}
            title={isCollapsed ? "Expandir menú" : undefined}
          >
            <Flame className="w-5 h-5 text-white" />
          </button>
          <span className={`font-bold text-xl tracking-tight transition-all duration-300 overflow-hidden whitespace-nowrap ${isCollapsed ? "w-0 opacity-0" : "w-16 opacity-100"
            }`}>
            C&F
          </span>
        </div>
        ...
        {filteredItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`font-medium transition-all duration-300 overflow-hidden whitespace-nowrap flex-1 ${isCollapsed ? "w-0 opacity-0" : "w-28 opacity-100"
                }`}>
                {item.name}
              </span>
